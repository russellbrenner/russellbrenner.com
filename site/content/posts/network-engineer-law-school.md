...
So I built a retrieval system that understands legal language natively. A RAG API using voyage-law-2 embeddings (trained on legal corpora, not general text) with citation-aware chunking that preserves the structure of judgments and legislation. It runs on k3s. The system knows that *Perre v Apand Pty Ltd* is a leading authority on pure economic loss not because someone tagged it, but because the embedding model learned the structural signals that mark a case as authoritative.
